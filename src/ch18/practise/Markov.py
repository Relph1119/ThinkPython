...
from ch13.ex8.markov import skip_gutenberg_header, shift
...
def main(script, filename=os.path.dirname(sys.path[2]) + os.sep + "resource"+ os.sep +"emma.txt", n=100, order=2):
    try:
        n = int(n)
        order = int(order)
    except ValueError:
